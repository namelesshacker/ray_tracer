...
Ray::Ray(double r, double alfa, double beta)
		{
...
double Ray::rayTrace(Vector &origin, Vector &unit, Sphere balls[], int n, double altitute, double coeff, int limit)
{
    double color;
...
/**
 * Raytrace all objects instead of only the spheres
*/
double Ray::rayTraceObject(Vector &origin, Vector &unit, Sphere balls[], int n, double altitute, double coeff, int limit)
{
    double color;
    double distanceToPlane = -(origin.getZ() + altitute) / unit.getZ(); /* this is signed distance */
    int index = -1;
